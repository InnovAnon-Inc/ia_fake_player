-- ia_fake_player/actions/primitive/pickup.lua
...
function ia_fake_player.actions.primitive.pickup_item(self, item_obj)
	minetest.log('ia_fake_player.actions.primitive.pickup_item()')
...
function ia_fake_player.actions.primitive.pickup_nearby(self, radius, condition)
	minetest.log('ia_fake_player.actions.primitive.pickup_nearby()')
    local items = self:find_items(radius, condition) -- Calls the sensor helper
    local any_picked_up = false

    for _, item_data in ipairs(items) do
        if ia_fake_player.actions.primitive.pickup_item(self, item_data.object) then
            any_picked_up = true
        end
    end
