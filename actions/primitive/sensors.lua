-- ia_fake_player/actions/primitive/sensors.lua
...
function ia_fake_player.actions.primitive.find_players(self, radius, condition)
...
function ia_fake_player.actions.primitive.find_items(self, radius, condition)
...
function ia_fake_player.actions.primitive.find_entities(self, radius, condition)
...
function ia_fake_player.actions.primitive.find_reachable_item(self, radius, condition)
	minetest.log('ia_fake_player.actions.find_reachable_item()')
    -- 1. Get all items sorted by distance (your existing code)
    local items = ia_fake_player.actions.primitive.find_items(self, radius, condition)
    
    -- 2. Iterate through sorted list and return the first reachable one
    for _, item_data in ipairs(items) do
        if ia_fake_player.actions.primitive.is_target_accessible(item_data.pos) then
...
function ia_fake_player.actions.primitive.is_not_crowded(stack, obj) -- TODO expose convenience filters
    ...
    return not ia_fake_player.actions.primitive.is_node_occupied(pos, obj)
end
...
--function ia_fake_player.actions.primitive.get_sorted_nodes(pos, radius, node_names)
...
function ia_fake_player.actions.primitive.get_headspace(self, max_dist)
    local pos = self:get_pos()
    ...
    for i = 1, max_dist do
        ...
        if ia_fake_player.actions.primitive.get_node_properties(node.name) then -- if walkable/solid
            return i - 1
        end
    end
    return max_dist
...
function ia_fake_player.actions.primitive.is_indoors(self)
    local pos = self:get_pos()
    ...
    return (light_sun or 0) < 5 and ia_fake_player.actions.primitive.get_headspace(self, 15) < 15
end

--- Generic Danger Detection
-- Returns a threat score (0 to 100).
function ia_fake_player.actions.primitive.get_danger_level(self)
    local pos = self:get_pos()
    if not pos then return 0 end
    ...
    for _, obj in ipairs(nearby) do
        if obj ~= self.object and ia_fake_player.actions.primitive.is_hostile(self, obj) then -- TODO
...
function ia_fake_player.actions.primitive.is_in_danger(self)
    return ia_fake_player.actions.primitive.get_danger_level(self) > 10
end
...
function ia_fake_player.actions.primitive.default_sort(a, b)
    return a.distance < b.distance
end
...
-- @return table Sorted list of {object = obj, pos = p, distance = d}
function ia_fake_player.actions.primitive.get_sorted_objects(pos, radius, filter_func, sort_func)
    local all_objects = minetest.get_objects_inside_radius(pos, radius)
    local filtered = {}
    ...
    table.sort(filtered, sort_func or ia_fake_player.actions.primitive.default_sort)
    return filtered
end

--- Finds all nodes in area with custom sorting support.
-- @param sort_func Optional function(a, b) to preference specific axes.
function ia_fake_player.actions.primitive.get_sorted_nodes(pos, radius, node_names, sort_func)
    local minp = vector.add(pos, -radius)
    local maxp = vector.add(pos, radius)
    local nodes = minetest.find_nodes_in_area(minp, maxp, node_names)
    ...
    table.sort(sorted, sort_func or ia_fake_player.actions.primitive.default_sort)
    return sorted
end
...
function ia_fake_player.actions.primitive.find_inventories(self, radius, check_items_func, sort_func)
    local pos = self:get_pos()
    -- Common container groups in Minetest
    local node_names = {"group:container", "group:chest", "group:furnace"}
    local nodes = ia_fake_player.actions.primitive.get_sorted_nodes(pos, radius, node_names, sort_func)
...
-- Usage: self:find_inventories(10, nil, ia_fake_player.actions.primitive.create_y_penalty_sort(pos, 2.0))
function ia_fake_player.actions.primitive.create_y_penalty_sort(center_pos, y_weight)
    return function(a, b)
