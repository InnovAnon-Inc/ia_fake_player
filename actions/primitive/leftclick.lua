-- ia_fake_player/actions/primitive/leftclick.lua
...
function ia_fake_player.actions.primitive.can_dig_node(self, node_name)
    local def = minetest.registered_nodes[node_name]
...
function ia_fake_player.actions.primitive.left_click(self, pos_or_obj, keep_tool)
    minetest.log('ia_fake_player.actions.primitive.left_click()')
    local is_object = type(pos_or_obj) == "userdata" and pos_or_obj.get_pos
    local pos = is_object and pos_or_obj:get_pos() or pos_or_obj

    if not ia_fake_player.actions.primitive.is_within_reach(self, pos) then return false, "too_far" end

    ia_fake_player.actions.primitive.stop_and_look_at(self, pos)
    ia_fake_player.actions.primitive.set_animation(self, 'MINE')
    ...
    if is_object then
        ...
        if success then ia_fake_player.actions.primitive.play_node_sound(node.name, pos, "dug") end
    end
