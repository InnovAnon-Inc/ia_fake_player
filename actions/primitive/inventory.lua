-- ia_fake_player/actions/primitive/inventory.lua
...
-- @return boolean.
function ia_fake_player.actions.primitive.has_item(self, condition)
	minetest.log('ia_fake_player.actions.primitive.has_item()')
...
function ia_fake_player.actions.primitive.add_to_inventory(self, stack)
	minetest.log('ia_fake_player.actions.primitive.add_to_inventory()')
    return self:get_inventory():add_item("main", stack)
end

--- Checks if the Dunce has room for at least one of this item.
function ia_fake_player.actions.primitive.has_room_for(self, item_name)
	minetest.log('ia_fake_player.actions.primitive.has_room_for()')
    --local inv = self:get_inventory()
    local inv = self:get_inventory()
    assert(inv ~= nil)
...
function ia_fake_player.actions.primitive.get_item_count(self, item_name)
	minetest.log('ia_fake_player.actions.primitive.get_item_count()')
    local inv = self:get_inventory()
    local count = 0
...
function ia_fake_player.actions.primitive.is_inventory_full(self, list_name)
    local inv = self:get_inventory()
