-- ia_fake_player/actions/primitive/fall.lua

--- Logic for detecting and reacting to falling.
function ia_fake_player.actions.primitive.fall(self)
	--minetest.log('ia_fake_player.actions.primitive.fall()')
    ...
    if v.y < -4.0 then
        ...
        ia_fake_player.actions.primitive.set_animation(self, 'FALL')
...
function ia_fake_player.actions.primitive.is_falling(self)
    local v = self:get_velocity()
