...
function ia_fake_player.actions.primitive.can_jump_to(self, target_pos)
    ...
    local is_walkable = ia_fake_player.actions.primitive.get_node_properties(minetest.get_node(below).name)
    if not is_walkable then return false end

    -- 3. Is there a physical obstacle in front of us that requires a jump?
    local ahead_pos = ia_fake_player.actions.primitive.get_relative_node_pos(self, 1, 0)
    local _, height = ia_fake_player.actions.primitive.get_node_properties(minetest.get_node(ahead_pos).name)
    
    -- 4. Is the head-space clear for the jump?
    local head_pos = ia_fake_player.actions.primitive.get_relative_node_pos(self, 1, 1)
    local head_clear = ia_fake_player.actions.primitive.is_buildable(head_pos)
...
function ia_fake_player.actions.primitive.jump(self)
    ...
    if math.abs(v.y) < 0.1 then
        ...
        ia_fake_player.actions.primitive.apply_vertical_impulse(self, 4.5) -- Adjusted for standard MTG jump
        return true
    end
