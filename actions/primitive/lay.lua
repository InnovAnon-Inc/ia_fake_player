-- ia_fake_player/actions/primitive/lay.lua

--function ia_fake_player.actions.primitive.lay(self)
--	minetest.log('ia_fake_player.actions.primitive.lay()')
--    -- Reduce collision box height for laying down
--    self:set_properties({
--        collisionbox = {-0.3, 0, -0.3, 0.3, 0.5, 0.3},
--    })
--    ia_fake_player.actions.primitive.set_animation(self, 'LAY')
--end
--
----- Sets the mob into a laying posture.
---- @param self The entity object.
---- @param state boolean: true to lay down, false to stand up.
--function ia_fake_player.actions.primitive.set_lay_posture(self, state)
--	minetest.log('ia_fake_player.actions.primitive.set_lay_posture()')
--    if state then
