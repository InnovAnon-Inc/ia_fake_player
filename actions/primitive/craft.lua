-- ia_fake_player/actions/primitive/craft.lua

--- Helper to turn a flat recipe list into a count table.
function ia_fake_player.actions.primitive.get_recipe_requirements(recipe_items)
    local reqs = {}
...
function ia_fake_player.actions.primitive.has_required_items(self, requirements)
	minetest.log('ia_fake_player.actions.primitive.has_required_items()')
    for item_name, amount in pairs(requirements) do
        if type(amount) == "number" then
            if ia_fake_player.actions.primitive.get_item_count(self, item_name) < amount then
                return false
            end
        end
    end
...
-- @return boolean.
function ia_fake_player.actions.primitive.can_obtain_item(self, item_name)
    -- 1. Do we already have it?
    if ia_fake_player.actions.primitive.has_item(self, item_name) then
        return true
    end
    ...
    if recipe and recipe.items then
        local reqs = ia_fake_player.actions.primitive.get_recipe_requirements(recipe.items)
        if ia_fake_player.actions.primitive.has_required_items(self, reqs) then
            return true
        end
    end
...
function ia_fake_player.actions.primitive.craft_item(self, output_name)
    minetest.log('ia_fake_player.actions.primitive.craft_item(' .. output_name .. ')')
    ...
    if not ia_fake_player.actions.primitive.has_required_items(self, ia_fake_player.actions.primitive.get_recipe_requirements(recipe.items)) then
    ...
    -- Feedback
    ia_fake_player.actions.primitive.set_animation(self, "MINE", 20, false)
