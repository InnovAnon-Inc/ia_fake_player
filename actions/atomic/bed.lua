-- ia_fake_player/actions/atomic/bed.lua
-- NOTE must handle beds and sleeping mats
--
------ Helper: Get the direction vector from param2
----local function get_direction_from_param2(param2)
----    local dirs = {
----        [0] = {x = 0, z = 1},   -- North
----        [1] = {x = 1, z = 0},   -- East
----        [2] = {x = 0, z = -1},  -- South
----        [3] = {x = -1, z = 0},  -- West
----    }
----    return dirs[param2 % 4]
----end
----
------- Makes the mob lay down, automatically adjusting for bed parts.
------ @param self The fake player object
------ @param pos The position of the node (Head, Foot, or Mat)
----function ia_fake_player.lay_down(self, pos)
----	minetest.log('ia_fake_player.lay_down()')
----    local node = minetest.get_node(pos)
----    local def = minetest.registered_nodes[node.name]
----    if not def then return false end
----
----    local is_mat = node.name == "sleeping_mat:mat"
----    local target_pos = vector.new(pos)
----    local dir = get_direction_from_param2(node.param2)
----
----    -- 1. Offset Logic: Find the "Head" if we clicked the "Foot"
----    -- Standard beds use groups to identify parts.
----    if not is_mat and minetest.get_item_group(node.name, "is_bed_foot") ~= 0 then
----        -- Move target_pos to the head node based on rotation
----        target_pos = vector.add(pos, dir)
----    end
----
----    -- 2. Physics & State
----    self:set_velocity({x = 0, y = 0, z = 0})
----    self:set_properties({ physical = false })
----
----    -- 3. Positioning
----    -- Mats are flat on the floor, beds are raised.
----    local height_offset = is_mat and 0.1 or 0.4
----    self:set_pos({x = target_pos.x, y = target_pos.y + height_offset, z = target_pos.z})
----
----    -- 4. Rotation
----    -- We want the mob to face AWAY from the headboard (laying down)
----    -- Or align with the mat's long axis.
----    local yaw = math.atan2(dir.x, dir.z) + math.pi
----    self:set_yaw(yaw)
----
----    -- 5. Animation (LAY sequence)
----    --self:set_animation({x = 162, y = 166}, 0, 0, false)
----    ia_fake_player.set_animation(self, 'LAY')
----    
----    self.data.is_sleeping = true
----    return true
----end
----
----function ia_fake_player.get_up(self)
----	minetest.log('ia_fake_player.get_up()')
----    self:set_properties({ physical = true })
----    --self:set_animation({x = 0, y = 79}, 30, 0, true) -- STAND
----    ia_fake_player.set_animation(self, 'STAND')
----
----    -- Shift them slightly so they don't spawn 'inside' the bed frame
----    local pos = self:get_pos()
----    self:set_pos({x = pos.x, y = pos.y + 0.5, z = pos.z})
----
----    self.data.is_sleeping = false
----    return true
----end
--
---- Helper: Get the direction vector from param2
--
--
----- Makes the mob lay down, automatically adjusting for bed parts.
--function ia_fake_player.lay_down(self, pos) -- TODO in lay.lua
--	minetest.log('ia_fake_player.lay_down()')
--    local node = minetest.get_node(pos)
--    local def = minetest.registered_nodes[node.name]
--    if not def then return false end
--
--    local is_mat = node.name == "sleeping_mat:mat"
--    local target_pos = vector.new(pos)
--    local dir = get_direction_from_param2(node.param2)
--
--    -- 1. Offset Logic: Find the "Head"
--    if not is_mat and minetest.get_item_group(node.name, "is_bed_foot") ~= 0 then
--        target_pos = vector.add(pos, dir)
--    end
--
--    -- 2. State & Physics
--    ia_fake_player.stop(self)
--    ia_fake_player.set_lay_posture(self, true)
--
--    -- 3. Positioning
--    local height_offset = is_mat and 0.05 or 0.4
--    self:set_pos({x = target_pos.x, y = target_pos.y + height_offset, z = target_pos.z})
--
--    -- 4. Rotation: Align with bed axis
--    local yaw = math.atan2(dir.x, dir.z) + math.pi
--    self:set_yaw(yaw)
--
--    self.data.is_sleeping = true
--    return true
--end
--
----- Handles waking up and clearing the bed obstruction.
--function ia_fake_player.get_up(self) -- TODO in lay.lua
--	minetest.log('ia_fake_player.get_up()')
--    if not self.data.is_sleeping then return false end
--
--    local current_pos = self:get_pos()
--
--    -- 1. Restore Physics and Animation
--    ia_fake_player.set_lay_posture(self, false)
--
--    -- 2. Edge Case: Obstruction Handling
--    -- Find a safe spot so we don't stand up "into" the wall or bed headboard
--    local safe_pos = find_safe_wakeup_pos(vector.round(current_pos))
--    self:set_pos(safe_pos)
--
--    self.data.is_sleeping = false
--    return true
--end
--
--
----- Finds the closest bed or sleeping mat.
--function ia_fake_player.find_closest_bed(self, radius)
--    local pos = self:get_pos()
--    local beds = ia_fake_player.get_sorted_nodes(pos, radius, {"group:bed", "sleeping_mat:mat"})
--    return beds[1] -- Return closest
--end
-- ia_fake_player/actions/atomic/bed.lua

--- Low-level: Is this node a bed or mat?
function ia_fake_player.actions.atomic.is_bed(pos)
    local node = minetest.get_node(pos)
    return minetest.get_item_group(node.name, "bed") > 0 or node.name == "sleeping_mat:mat"
end

--- Mid-level: Is there a bed we can actually use?
-- Checks for ownership, headspace, and valid bed parts.
function ia_fake_player.actions.atomic.can_sleep(self, pos)
    if not ia_fake_player.actions.atomic.is_bed(pos) then return false end
    
    -- Check for headspace (2 nodes of air above the bed)
    local above_1 = {x=pos.x, y=pos.y+1, z=pos.z}
    local above_2 = {x=pos.x, y=pos.y+2, z=pos.z}
    if minetest.get_node(above_1).name ~= "air" then return false end

    -- Ownership check (if the mod supports it)
    local meta = minetest.get_meta(pos)
    local owner = meta:get_string("owner")
    if owner ~= "" and owner ~= self.name then return false end

    return true
end

--- High-level: Could we sleep if we crafted/placed a bed?
function ia_fake_player.actions.atomic.could_sleep(self, pos)
    -- 1. Can we sleep right now?
    if ia_fake_player.actions.atomic.can_sleep(self, pos) then return true end

    -- 2. Could we place one at this location?
    -- Requires 2 blocks of flat space and 2 blocks of air.
    if ia_fake_player.actions.primitive.is_buildable(pos) and ia_fake_player.actions.primitive.can_obtain_item(self, "group:bed") then
        return true
    end

    return false
end

--- Level 1: Atomic Action (Lay down on existing bed)
function ia_fake_player.actions.atomic.lay_down(self, pos)
    minetest.log('ia_fake_player.lay_down()')
    local node = minetest.get_node(pos)
    local dir = ia_fake_player.actions.primitive.get_direction_from_param2(node.param2)
    local target_pos = vector.new(pos)

    -- Offset Logic: Find the "Head" if we clicked the "Foot"
    if minetest.get_item_group(node.name, "is_bed_foot") ~= 0 then
        target_pos = vector.add(pos, dir)
    end

    ia_fake_player.actions.primitive.stop(self)
    ia_fake_player.actions.primitive.set_lay_posture(self, true)

    -- Mats are flat, beds are raised
    local is_mat = node.name == "sleeping_mat:mat"
    local height_offset = is_mat and 0.05 or 0.4
    
    self:set_pos({x = target_pos.x, y = target_pos.y + height_offset, z = target_pos.z})
    self:set_yaw(math.atan2(dir.x, dir.z) + math.pi)

    self.data.is_sleeping = true
    return true
end

--- Level 2: Preparation + Action (Find/Place then Sleep)
function ia_fake_player.actions.atomic.place_and_sleep(self, pos)
    minetest.log('ia_fake_player.place_and_sleep()')
    
    local is_bed_item = function(n) return minetest.get_item_group(n, "bed") > 0 end
    if ia_fake_player.actions.primitive.wield_by_condition(self, is_bed_item) then
        local success = ia_fake_player.actions.primitive.right_click(self, pos, false)
        if success then
            return ia_fake_player.actions.primitive.lay_down(self, pos)
        end
    end
    return false
end

--- Level 3: Provisioning + Preparation + Action
function ia_fake_player.actions.atomic.craft_and_sleep(self, pos)
    minetest.log('ia_fake_player.craft_and_sleep()')
    
    if not ia_fake_player.actions.primitive.has_item(self, function(n) return minetest.get_item_group(n, "bed") > 0 end) then
        if ia_fake_player.actions.primitive.can_obtain_item(self, "default:bed") then
            ia_fake_player.actions.primitive.craft_item(self, "default:bed")
        end
    end
    
    return ia_fake_player.actions.atomic.place_and_sleep(self, pos)
end

--- Handles waking up and clearing the bed obstruction.
function ia_fake_player.get_up(self)
    minetest.log('ia_fake_player.get_up()')
    if not self.data.is_sleeping then return false end

    ia_fake_player.actions.primitive.set_lay_posture(self, false)

    -- Move to a safe adjacent spot to avoid getting stuck in the bed node
    local safe_pos = ia_fake_player.actions.primitive.find_safe_wakeup_pos(vector.round(self:get_pos()))
    self:set_pos(safe_pos)

    self.data.is_sleeping = false
    return true
end
