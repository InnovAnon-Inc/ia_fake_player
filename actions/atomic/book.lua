-- ia_fake_player/actions/atomic/book.lua
-- TODO ia_missionary
...
function ia_fake_player.actions.atomic.is_readable(pos)
...
function ia_fake_player.actions.atomic.can_read(self, pos)
    if not ia_fake_player.actions.atomic.is_readable(pos) then return false end
    
    -- Interaction usually requires being close enough
    return ia_fake_player.actions.primitive.is_within_reach(self, pos)
end

--- High-level: Could the agent read this if they moved or obtained necessary items?
function ia_fake_player.actions.atomic.could_read(self, pos)
    -- If it's a book, we can read it once we reach it.
    return ia_fake_player.actions.atomic.is_readable(pos)
end

--- Level 1: Atomic Action
-- Performs the right-click/use interaction on a book node.
function ia_fake_player.actions.atomic.read_book(self, pos)
    minetest.log('ia_fake_player.read_book()')
...
function ia_fake_player.actions.atomic.equip_and_read(self, pos)
    minetest.log('ia_fake_player.equip_and_read()')
    if not ia_fake_player.actions.atomic.can_read(self, pos) then
        return false, "out_of_reach_or_invalid"
    end

    ia_fake_player.actions.primitive.stop_and_look_at(self, pos)
    return ia_fake_player.actions.atomic.read_book(self, pos)
end
...
function ia_fake_player.actions.atomic.obtain_and_read(self, pos)
    minetest.log('ia_fake_player.obtain_and_read()')
    
    -- If we aren't at the book, we would usually trigger pathfinding here. -- FIXME obtain means craft. we don't do pathfinding here
    -- Assuming the agent has reached the destination:
    return ia_fake_player.actions.atomic.equip_and_read(self, pos)
end

--- Helper: Check if an item in inventory is a book.
function ia_fake_player.actions.atomic.is_book_item(item_name)
