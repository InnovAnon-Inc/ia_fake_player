...
function ia_fake_player.actions.atomic.is_trapdoor(pos)
    local node = minetest.get_node(pos)
    return minetest.get_item_group(node.name, "trapdoor") > 0
end

--- Is the trapdoor currently open?
function ia_fake_player.actions.atomic.is_trapdoor_open(pos)
...
function ia_fake_player.actions.atomic.can_open_trapdoor(self, pos)
    if not ia_fake_player.actions.atomic.is_trapdoor(pos) then return false end
    ...
    if is_steel then
        return dist < 1.5 and ia_fake_player.actions.primitive.has_item(self, "default:key") -- Placeholder logic
    end
...
function ia_fake_player.actions.atomic.could_use_trapdoor(self, pos)
    if ia_fake_player.actions.atomic.is_trapdoor(pos) then return true end
    
    -- Could we place one here?
    local node = minetest.get_node(pos)
    local def = minetest.registered_nodes[node.name]
    if def and not def.walkable and ia_fake_player.actions.primitive.can_obtain_item(self, "group:trapdoor") then
        return true
    end
...
function ia_fake_player.actions.atomic.interact_trapdoor(self, pos, action)
    minetest.log('action', '[ia_fake_player] interact_trapdoor: ' .. (action or "toggle"))
...
function ia_fake_player.actions.atomic.prepare_trapdoor_path(self, pos, target_state)
    local current_open = ia_fake_player.actions.atomic.is_trapdoor_open(pos)
    
    if current_open ~= target_state then
        return ia_fake_player.actions.atomic.interact_trapdoor(self, pos, target_state and "open" or "close")
    end
...
function ia_fake_player.actions.atomic.place_and_use_trapdoor(self, pos)
    if not ia_fake_player.actions.primitive.has_item(self, "group:trapdoor") then
        if ia_fake_player.actions.primitive.can_obtain_item(self, "doors:trapdoor") then
            ia_fake_player.actions.primitive.craft_item(self, "doors:trapdoor")
        end
    end
    
    -- Wield and place
    if ia_fake_player.actions.primitive.wield_by_condition(self, function(s) return minetest.get_item_group(s:get_name(), "trapdoor") > 0 end) then
...
local function set_trapdoor_state(pos, state)
    minetest.log('info', '[ia_fake_player] set_trapdoor_state() at ' .. minetest.pos_to_string(pos))
    if not minetest.get_modpath("doors") then return false end

    local door = doors.get(pos)
    if not door then
        minetest.log('warning', '[ia_fake_player] Could not get trapdoor object at ' .. minetest.pos_to_string(pos))
        return false
    end
...
function ia_fake_player.actions.atomic.get_trapdoor_vectors(pos)
    local node = minetest.get_node(pos)
...
function ia_fake_player.actions.atomic.handle_trapdoor(self, action, direction)
    minetest.log('info', 'ia_fake_player.handle_trapdoor()')
...
function ia_fake_player.actions.atomic.find_nearby_trapdoors(pos, radius)
    local r = radius or 2
    return ia_fake_player.actions.primitive.get_sorted_nodes(pos, r, {"group:trapdoor"})
end
