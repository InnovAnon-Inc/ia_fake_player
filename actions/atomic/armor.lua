-- ia_fake_player/actions/atomic/armor.lua
-- NOTE must handle boots, pants, shirt, helmet, shield and whatever the 6th slot is for

----- Low-level: Is this item name actually armor?
---- @param item_name The name of the item.
---- @param armor_type Optional specific group like "torso", "legs", "head", "feet".
--function ia_fake_player.actions.atomic.is_armorable(item_name, armor_type)
...
--function ia_fake_player.actions.atomic.is_armored(self, armor_type)
...
--        if not stack:is_empty() and ia_fake_player.actions.atomic.is_armorable(stack:get_name(), armor_type) then
--            return true
--        end
--    end
--    return false
...
function ia_fake_player.actions.atomic.could_armor(self, armor_type)
    if ia_fake_player.actions.atomic.can_armor(self, armor_type) then return true end
...
function ia_fake_player.actions.atomic.equip_armor_stack(self, stack)
    minetest.log('ia_fake_player.actions.atomic.equip_armor_stack()')
...
--function ia_fake_player.actions.atomic.search_and_equip_armor(self, armor_type)
--    minetest.log('ia_fake_player.actions.atomic.search_and_equip_armor()')
--    local inv = self:get_inventory()
--    local main_list = inv:get_list("main")
--
--    for i, stack in ipairs(main_list) do
--        if not stack:is_empty() and ia_fake_player.actions.atomic.is_armorable(stack:get_name(), armor_type) then
--            local success = ia_fake_player.actions.atomic.equip_armor_stack(self, stack)
--            if success then
...
--function ia_fake_player.actions.atomic.is_armor(item_name)
...
-- Finds armor in inventory and puts it on.
function ia_fake_player.actions.atomic.search_and_equip_armor(self, armor_type)
    minetest.log('action', '[ia_fake_player] search_and_equip_armor() looking for ' .. (armor_type or "any armor"))
    ...
    for i, stack in ipairs(main_list) do
        if not stack:is_empty() and ia_fake_player.actions.atomic.is_armorable(stack:get_name(), armor_type) then
            local success, err = ia_fake_player.actions.atomic.equip_armor_stack(self, stack)
            if success then
                ...
                minetest.log('warning', '[ia_fake_player] Failed to equip ' .. stack:get_name() .. ': ' .. tostring(err))
            end
        end
    end
    return false, "no_armor_found_in_inventory"
...
function ia_fake_player.actions.atomic.craft_and_equip_armor(self, armor_type)
    minetest.log('ia_fake_player.actions.atomic.craft_and_equip_armor()')
    ...
    return ia_fake_player.actions.atomic.search_and_equip_armor(self, armor_type)
end
...
--- High-level: Returns a table of armor requirements for missing slots.
function ia_fake_player.actions.atomic.get_armor_requirements(self) -- primitive ?
    local missing = {}
    local elements = {"head", "torso", "legs", "feet"}
    local found_missing = false

    for _, element in ipairs(elements) do
        if not ia_fake_player.actions.atomic.is_armored(self, element) then
