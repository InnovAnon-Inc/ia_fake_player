-- ia_fake_player/actions/simple/punch.lua

function ia_fake_player.actions.simple.punch(self, target)
    minetest.log('ia_fake_player.actions.simple.punch()')
    local target_pos = (type(target) == "table" and target.x) and target or target:get_pos()
    if not target_pos then return false end

    ia_fake_player.actions.simple.stop_and_look_at(self, target_pos)
    ...
    ia_fake_player.actions.simple.set_animation(self, 'PUNCH', 30, false)
