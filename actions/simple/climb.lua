...
function ia_fake_player.actions.simple.is_at_climb_down_target(self, target_y)
    local pos = self:get_pos()
    if not pos then return false end
...
function ia_fake_player.actions.simple.climb_up(self, target_y)
    local pos = self:get_pos()
    if not pos or not ia_fake_player.actions.simple.is_climbable(pos) then
    ...
    if not ia_fake_player.actions.simple.is_climbable(above) and pos.y >= (target_y - 0.5) then
    ...
    if not self._is_climbing then
        ia_fake_player.actions.simple.set_animation(self, 'CLIMB')
...
function ia_fake_player.actions.simple.climb_down(self, target_y)
    local pos = self:get_pos()
    if not pos or not ia_fake_player.actions.simple.is_climbable(pos) then
        self._is_climbing = false
        return false
...
function ia_fake_player.actions.simple.climb(self, target_y)
    local pos = self:get_pos()
    if not pos then return false end

    if target_y and target_y < pos.y - 0.5 then
        return ia_fake_player.actions.simple.climb_down(self, target_y)
    else
        -- Default to up if no target or target is above
        return ia_fake_player.actions.simple.climb_up(self, target_y or (pos.y + 1))
    end
end
