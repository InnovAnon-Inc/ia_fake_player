...
-- @param detached_name The name of the detached inventory
-- @param listname The specific list to sync (e.g., "main")
function ia_fake_player:sync_to_detached(fake_inv, detached_name, listname) -- TODO skip "armor" ?
    --minetest.log('ia_fake_player sync_to_detached detached_name: '..tostring(detached_name)..', listname='..tostring(listname))
    local detached = minetest.get_inventory({type="detached", name=detached_name})
    --local detached = minetest.get_inventory({type="detached", name=minetest.formspec_escape(detached_name)}) -- TODO testing
    if not detached then return end
...
-- @return The string name of the created detached inventory
function ia_fake_player:create_mirror(player_name, fake_inv)
    minetest.log('ia_fake_player.create_mirror(playername='..tostring(player_name)..')')
    assert(player_name, "create_mirror: player_name is required")
    assert(fake_inv, "create_mirror: fake_inv is required")

    -- Use the player_name directly to match formspec: list[detached:<name>;...]
    local detached_name = player_name
    --minetest.log('ia_fake_player create_mirror detached_name: '..tostring(detached_name))

--    local detached = minetest.create_detached_inventory(detached_name, {
--        allow_put = function(inv, listname, index, stack, player) -- TODO perms or IoC
--            minetest.log('ia_fake_player detached_inventory allow_put')
--            return stack:get_count()
--        end,
--	allow_take = function(inv, listname, index, stack, player) -- TODO perms or IoC
--            minetest.log('ia_fake_player detached_inventory allow_take')
--            return stack:get_count()
--        end,
--	allow_move = function(inv, from_list, from_index, to_list, to_index, count, player) -- TODO perms or IoC
--            minetest.log('ia_fake_player detached_inventory allow_move')
--            return count -- Explicitly allow the move
--        end,
--        on_put = function(inv, listname, index, stack, player)
--            minetest.log('ia_fake_player detached_inventory on_put')
--            -- Sync change back to the source of truth (fakelib)
--            fake_inv:set_stack(listname, index, stack)
--        end,
--        on_take = function(inv, listname, index, stack, player)
--            minetest.log('ia_fake_player detached_inventory on_take')
--            fake_inv:set_stack(listname, index, ItemStack(""))
--        end,
--        --on_move = function(inv, from_list, from_index, to_list, to_index, count, player)
--        --    local stack = inv:get_stack(to_list, to_index)
...
--        --end,
--	on_move = function(inv, from_list, from_index, to_list, to_index, count, player)
--            minetest.log('ia_fake_player detached_inventory on_move')
--            local stack = inv:get_stack(to_list, to_index) -- This is the stack AFTER the move
--            fake_inv:set_stack(from_list, from_index, inv:get_stack(from_list, from_index)) -- Sync the now-empty/reduced source
--            fake_inv:set_stack(to_list, to_index, stack) -- Sync the target
--        end
--    --}, player_name) -- NOTE testing
--    --}, nil)
--    })
    --assert(minetest.get_inventory({type="detached", name=detached_name}) == nil)
    local detached = minetest.create_detached_inventory(detached_name, {
    --local detached = minetest.create_detached_inventory(minetest.formspec_escape(detached_name), { -- TODO testing
        allow_put = function(inv, listname, index, stack, player)
            return stack:get_count()
        end,
        ...
        on_take = function(inv, listname, index, stack, player)
            fake_inv:set_stack(listname, index, inv:get_stack(listname, index))
        end,
        on_move = function(inv, from_list, from_index, to_list, to_index, count, player)
            fake_inv:set_stack(from_list, from_index, inv:get_stack(from_list, from_index))
            fake_inv:set_stack(to_list, to_index, inv:get_stack(to_list, to_index))
        end,
    }) -- No third argument, allows public access
