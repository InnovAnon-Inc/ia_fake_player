...
--function ia_fake_player.init_entity_state(entity, staticdata) -- not used anywhere; project works fine without it... necessary ?
...
function ia_fake_player.cleanup_entity(entity)
    ...
    local data = entity.data -- TODO skip "armor" ?
    if data and data.detached_name then
        ...
        --local inv = minetest.get_inventory({type="detached", name=minetest.formspec_escape(data.detached_name)}) -- TODO testing
        if inv then
